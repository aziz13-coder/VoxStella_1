...
class SwissEphemerisAdapter:
    def __init__(self, *, ephemeris_path: Optional[str] = None, swe_module: Any = None):
        selected_module = swisseph if swe_module is None else swe_module
        self.swe = synchronized_swisseph(selected_module)
        if self.swe is None:
            raise RuntimeError("Swiss Ephemeris is unavailable")
        self.ephemeris_path = ephemeris_path or ""
    ...
    def longitude_at(self, dt_utc: datetime, body: str) -> float:
        ...
        with swisseph_ephemeris_path(self.ephemeris_path, swe_module=self.swe):
            jd_ut = self.swe.julday(
                dt.year,
                dt.month,
                dt.day,
                hour,
                getattr(self.swe, "GREG_CAL", 1),
            )
            try:
