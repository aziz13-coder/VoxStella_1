...
def time_to_perfection(
    ...
    IMPORTANT: Treat aspects with symmetry correctly. For non-axial aspects
    (sextile, square, trine), both +A and -A separations are equivalent
    (e.g., 90° and 270°). We therefore compute the minimal signed delta to
    the closest equivalent of the aspect geometry relative to the current
    signed separation.
    """

    # Current minimal signed separation in (-180°, 180°]
    rel_sep = ((pos1.longitude - pos2.longitude + 180) % 360) - 180
    A = aspect.degrees

    # Candidate target separations for this aspect
    if A == 0:
        targets = [0.0]
    elif A == 180:
        # Treat both representations explicitly to avoid edge-case drift
        targets = [180.0, -180.0]
    else:
        # Symmetric aspects: consider ±A (e.g., ±60, ±90, ±120)
    ...
    deltas = [norm180(rel_sep - t) for t in targets]
    # Choose the delta with smallest absolute magnitude
    delta = min(deltas, key=lambda d: abs(d))

    v_rel = pos1.speed - pos2.speed
    if abs(v_rel) < 1e-12:
        return math.inf
...
def _shift_to_next_future_perfection_time(
    pos1: PlanetPosition, pos2: PlanetPosition, aspect: Aspect, t: float
) -> float:
    """
    Return the earliest positive (future) perfection time corresponding to the same
    aspect geometry. This converts a non-positive 'nearest' root into the next
    forward root using the 360°/|v_rel| period.
    """
    ...
    # how many periods do we need to add to make t > 0?
        ...
        for planet2 in planet_list[i + 1 :]:
            ...
            for aspect_type in Aspect:
                ...
                if orb_diff <= max_orb:
                    ...
                    t_nearest = time_to_perfection(pos1, pos2, aspect_type)
                    # Keep output consistent: if applying, make sure the time we expose is future
                    t_for_output = (
                        _shift_to_next_future_perfection_time(pos1, pos2, aspect_type, t_nearest)
                        if applying_raw else t_nearest
                    )
                    if applying_raw:
                        within_sign = (
                            math.isfinite(t_for_output)
                            and t_for_output > 0
                            and _will_perfect_before_sign_exit(
                                pos1, pos2, aspect_type, t_for_output
                            )
                        )
                    else:
                        within_sign = False
                    ...
                    degrees_to_exact, exact_time = calculate_enhanced_degrees_to_exact(
                        pos1, pos2, aspect_type, jd_ut, t_for_output
                    )
                    ...
                    aspect_candidates.append(
                        AspectInfo(
                            ...
                            time_to_perfection=t_for_output,
...
def is_applying_enhanced(
    ...
    # Applying is determined kinematically (orb shrinking)
    applying = _is_orb_shrinking(pos1, pos2, aspect)
    # If applying but our analytical 'nearest' solution is not in the future,
    # shift to the next forward perfection to maintain self-consistency.
    if applying:
        t = _shift_to_next_future_perfection_time(pos1, pos2, aspect, t)
    if applying and math.isfinite(t) and t > 0:
        perfection_within_sign = _will_perfect_before_sign_exit(pos1, pos2, aspect, t)
    else:
...
def calculate_enhanced_degrees_to_exact(
    ...
    if t is None:
        t = time_to_perfection(pos1, pos2, aspect)
    # Use kinematics as source of truth for applying/separating
    applying = _is_orb_shrinking(pos1, pos2, aspect)
    # If applying but 't' is not future, shift it forward so exact_time is coherent
    if applying:
        t = _shift_to_next_future_perfection_time(pos1, pos2, aspect, t)
        max_future_days = cfg().timing.max_future_days
        if math.isfinite(t) and 0 < t <= max_future_days:
            try:
                exact_jd = jd_ut + t
        ...
        exact_time = None
        if math.isfinite(t) and t > 0:  # If somehow positive but not applying, make it negative
            t = -abs(t)
