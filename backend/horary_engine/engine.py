...
# Best-effort stdout debug gate for legacy print-based tracing.
_HORARY_DEBUG_STDOUT = str(os.getenv("HORARY_DEBUG_STDOUT", "")).strip().lower() in {"1", "true", "yes"}
_HORARY_DIAGNOSTIC_LOGS = str(os.getenv("HORARY_DEBUG_LOGS", "")).strip().lower() in {"1", "true", "yes"}
...
def _safe_debug_print(*args, **kwargs):
    ...
    try:
        ...
        logging.getLogger(__name__).debug("Suppressed horary engine debug print failure", exc_info=True)


print = _safe_debug_print  # type: ignore[assignment]


def _debug_log(level: int, message: str, *args) -> None:
    if not _HORARY_DIAGNOSTIC_LOGS:
        return
    logging.getLogger(__name__).log(level, message, *args)


# SHADOW MODE: Enable programmatically for testing new perfection system
# Set this to True to enable shadow mode comparison between old and new systems
ENABLE_SHADOW_MODE = True

# FULL PHASE: Use new perfection system exclusively (Phase 3 of migration)
# Set this to True to use new system for actual judgments while keeping shadow mode active
ENABLE_FULL_PHASE = True

if ENABLE_SHADOW_MODE:
    os.environ['HORARY_SHADOW_MODE'] = 'true'
    if ENABLE_FULL_PHASE:
        os.environ['HORARY_USE_NEW_PERFECTION'] = 'true'
        print("FULL PHASE ENABLED: New perfection system will be used for judgments")
        print("   - Shadow mode active for comparison logging")
        print("   - Judgment will use NEW system results")
        print("   - Set ENABLE_FULL_PHASE = False to revert to gradual phase")
    else:
        print("SHADOW MODE ENABLED: Both old and new perfection systems will run in parallel")
        print("   - Judgment will use OLD system (safe)")
        print("   - Comparison logs will show differences")
    print("   - Set ENABLE_SHADOW_MODE = False to disable")
else:
    print("SHADOW MODE DISABLED: Only old perfection system will run")
    print("   - Set ENABLE_SHADOW_MODE = True to enable testing")
...
def _structure_reasoning(reasoning: List[Any]) -> List[Dict[str, Any]]:
    ...
    for entry in reasoning:
        if isinstance(entry, dict):
            ...
            # NEW: preserve metadata used by the scorer / filters
            for k in ("key", "house", "factor", "kind", "family", "polarity", "aspect", "global"):
                if k in entry:
                    out[k] = entry[k]
        ...
        if m_paren_num:
            ...
            weight_str = m_paren_num.group(1)
        elif m_percent:
            weight_match = m_percent
            weight_str = m_percent.group(1)
        ...
        if weight_match:
            try:
                ...
                weight = 0
            rule = rule[: weight_match.start()].rstrip().rstrip(':').rstrip()
        else:
        ...
        # Derive stable key and lightweight hints for logging and filters
        derived_key = re.sub(r'[^a-z0-9_]+', '_', rule.lower())[:64] or 'text_item'
        entry_obj = {'stage': stage, 'rule': rule, 'weight': weight, 'key': derived_key}
        # Try to infer a house number from common tokens like L2, L7, etc.
        m = re.search(r'\bL(\d+)\b', rule, flags=re.I)
        if m:
            try:
                entry_obj['house'] = int(m.group(1))
            except Exception:
                pass
        # Heuristic factor tagging from wording to aid logs/filters
        rlow = rule.lower()
        if ('end of matter' in rlow) or ('end-of-matter' in rlow):
            entry_obj['factor'] = 'end_matter'
        elif 'cadent' in rlow:
            entry_obj['factor'] = 'cadent_significator'
        elif ('debilit' in rlow) or ('combust' in rlow) or ('peregrine' in rlow) or ('fall' in rlow):
            entry_obj['factor'] = 'house_condition'

        structured.append(entry_obj)
...
def _evaluate_enhanced(
    ...
    # Fix B: Reorder testimonies to show end_matter testimonies near the end (print order only)
    def sort_key(entry):
        factor = entry.get("factor", "")
        # end_matter testimonies go to end (higher sort value)
        # all others maintain original relative order (lower sort value)
        return (1, 0) if factor == "end_matter" else (0, 0)
    
    scoring_sorted = sorted(scoring, key=sort_key)
    
    for entry in scoring_sorted:
        ...
        # Global testimonies bypass all filters
        if entry.get("global"):
            print(f"      GLOBAL: bypassing all filters")
...
def extract_testimonies(chart: HoraryChart, contract: Dict[str, Planet]) -> List[Any]:
    ...
    for asp in getattr(chart, "aspects", []):
        ...
        if asp.applying and (asp.planet1 == Planet.MOON or asp.planet2 == Planet.MOON):
            other = actor2 if asp.planet1 == Planet.MOON else actor1
            role_name = other.name.lower() if isinstance(other, DslRole) else ""
            if asp.aspect is Aspect.TRINE and role_name == "examiner":
                primitives.append(TestimonyKey.MOON_APPLYING_TRINE_EXAMINER_SUN)
            elif asp.aspect is Aspect.SEXTILE:
            ...
            elif asp.aspect is Aspect.OPPOSITION:
                if role_name == "examiner":
    ...
    if sig1 and sig2 and sig1 in chart.planets and sig2 in chart.planets:
        ...
        if times:
            ...
            # Use perfection_core to detect events within the timeframe
            window_days = max(int(days_ahead + 1), 7)  # Minimum 7 days to catch events
            from .perfection_core import PerfectionCoreAPI, EventType
            core_api = PerfectionCoreAPI()
            core_result = core_api.find_perfections(chart, sig1, sig2, window_days)
            
            all_events = core_result.get("events", [])
            
            # Filter events that occur within days_ahead timeframe and convert to primitives
            for event in all_events:
                if hasattr(event, 'exact_in_days') and event.exact_in_days is not None:
                    if 0 < event.exact_in_days <= days_ahead:
                        # Convert core events to DSL primitives
                        if event.event_type == EventType.TRANSLATION:
                            reception = event.reception_data.get("type", "none") if event.reception_data else "none"
                            primitives.append(
                                dsl_translation(
                                    event.mediator,
                                    sig1,
                        ...
                        elif event.event_type == EventType.COLLECTION:
                            reception = event.reception_data.get("type", "none") if event.reception_data else "none"
                            primitives.append(
                                dsl_collection(
                                    event.mediator,
                        ...
                        elif event.event_type == EventType.ABSCISSION:
                            # For abscission, mediator is the abscissor, get significator from metadata
                            significator = event.metadata.get("interceptor") or event.primary_pair[0]
                            other_sig = sig1 if significator == sig2 else sig2
                            primitives.append(
                                dsl_abscission(
                                    event.mediator,
                                    significator,
                        ...
                        elif event.event_type == EventType.PROHIBITION:
                            # For prohibition, mediator is the prohibitor
                            significator = event.metadata.get("target") or event.primary_pair[0]
                            primitives.append(
                                dsl_prohibition(
                                    event.mediator,
                                    significator
    ...
    if not relevant_houses:
        ...
        elif chart_category == "lawsuit":
            relevant_houses.update({1, 7, 10, 4})  # Court axis: self, opponent, judge, outcome
    ...
    def _deg_in_sign(pos):
        try:
            ...
            return pos.longitude % 30

    weights = getattr(cfg(), "weights", None)
    # Safe defaults if config absent
    w_l1_bad = {6: -3, 8: -5, 12: -4}
    retro_pen  = getattr(weights, "retrograde_penalty", -2) if weights else -2
    ...
    if l1:
        ...
        if l1_pos and l1_pos.house in (6, 8, 12):
            base = w_l1_bad.get(l1_pos.house, -3)
            primitives.append({
                "key": "L1_IN_BAD_HOUSE",
                "rule": f"Querent (L1: {l1.value}) in {l1_pos.house} (adversity; 8th = fear/debt)",
                "weight": base,
                "house": l1_pos.house,
    ...
    # B) L2 condition (funds) with end-of-matter coloring
    ...
    if l2:
        ...
        if l2_pos:
            ...
            if l2_pos.house == 4:
                w = l2_in_4_base
                # Common intensifiers
                if getattr(l2_pos, "retrograde", False):
                    w += retro_pen
                if _deg_in_sign(l2_pos) >= 29.0:
                    w += anaretic_pen
                primitives.append({
...
try:
    ...
    from .property_doctrine import (
        derive_property_category_rules,
        build_property_advisability_snapshot,
        evaluate_property_advisability_snapshot,
    )
    from .economic_doctrine import (
        build_bank_counterparty_snapshot,
        evaluate_bank_counterparty_snapshot,
    )
    from .relationship_doctrine import (
        build_relationship_affection_snapshot,
        build_relationship_durability_snapshot,
        evaluate_relationship_affection_snapshot,
        evaluate_relationship_durability_snapshot,
    )
    from .pregnancy_doctrine import (
        build_pregnancy_conception_snapshot,
        build_pregnancy_diagnosis_snapshot,
        evaluate_pregnancy_conception_snapshot,
        evaluate_pregnancy_diagnosis_snapshot,
    )
    from .children_doctrine import (
        build_children_adoption_snapshot,
        evaluate_children_adoption_snapshot,
    )
    from .health_doctrine import (
        build_health_diagnosis_snapshot,
        build_health_progression_snapshot,
        evaluate_health_diagnosis_snapshot,
        evaluate_health_progression_snapshot,
    )
    from .pet_doctrine import (
        build_pet_missing_location_projection,
        build_pet_missing_snapshot,
        build_pet_recovery_snapshot,
        evaluate_pet_missing_snapshot,
        evaluate_pet_recovery_snapshot,
    )
    from .lost_object_doctrine import (
        build_lost_object_discovery_snapshot,
        build_lost_object_location_projection,
        evaluate_lost_object_discovery_snapshot,
    )
    from .competition_doctrine import (
        build_public_office_contest_snapshot,
        evaluate_public_office_contest_snapshot,
        build_champion_defense_snapshot,
    ...
    from category_rules import get_category_rules
    from horary_engine.property_doctrine import (
        derive_property_category_rules,
        build_property_advisability_snapshot,
        evaluate_property_advisability_snapshot,
    )
    from horary_engine.economic_doctrine import (
        build_bank_counterparty_snapshot,
        evaluate_bank_counterparty_snapshot,
    )
    from horary_engine.relationship_doctrine import (
        build_relationship_affection_snapshot,
        build_relationship_durability_snapshot,
        evaluate_relationship_affection_snapshot,
        evaluate_relationship_durability_snapshot,
    )
    from horary_engine.pregnancy_doctrine import (
        build_pregnancy_conception_snapshot,
        build_pregnancy_diagnosis_snapshot,
        evaluate_pregnancy_conception_snapshot,
        evaluate_pregnancy_diagnosis_snapshot,
    )
    from horary_engine.children_doctrine import (
        build_children_adoption_snapshot,
        evaluate_children_adoption_snapshot,
    )
    from horary_engine.health_doctrine import (
        build_health_diagnosis_snapshot,
        build_health_progression_snapshot,
        evaluate_health_diagnosis_snapshot,
        evaluate_health_progression_snapshot,
    )
    from horary_engine.pet_doctrine import (
        build_pet_missing_location_projection,
        build_pet_missing_snapshot,
        build_pet_recovery_snapshot,
        evaluate_pet_missing_snapshot,
    ...
    from horary_engine.lost_object_doctrine import (
        build_lost_object_discovery_snapshot,
        build_lost_object_location_projection,
        evaluate_lost_object_discovery_snapshot,
    )
    from horary_engine.competition_doctrine import (
        build_public_office_contest_snapshot,
        evaluate_public_office_contest_snapshot,
        build_champion_defense_snapshot,
        evaluate_champion_defense_snapshot,
...
from .perfection_core import PerfectionCoreAPI
from .aspects import (
...
class EnhancedTraditionalAstrologicalCalculator:
    ...
    def __init__(self, timezone_manager=None):
        ...
        # Traditional planets plus North Node
        self.planets_swe = {
            Planet.SUN: swe.SUN,
            ...
            Planet.NORTH_NODE: swe.MEAN_NODE
        }
    ...
    def calculate_chart(self, dt_local: datetime.datetime, dt_utc: datetime.datetime, 
                       ...
                       house_system_code: Optional[str] = None) -> HoraryChart:
        ...
        # Standard "orb-gated" next-Moon-aspect is removed.
        # Populate with VOC-mode (ignore orbs) so this field reflects the
        # traditional void-of-course logic only.
        moon_next_aspect = calculate_moon_next_aspect(
            planets, jd_ut, ignore_orb_for_voc=True
        )
        ...
        # Attach the house system code used for transparency/propagation
        try:
            setattr(chart, 'house_system_code', house_code if isinstance(house_code, str) else 'R')
        except Exception:
            pass
    ...
    def _analyze_enhanced_solar_condition(self, planet: Planet, planet_pos: PlanetPosition, 
        ...
        # Use configurable boundaries (optionally planet-specific), with legacy fallback.
        config = cfg()
        orbs_cfg = getattr(config, "orbs", SimpleNamespace())

        legacy_fixed = bool(getattr(orbs_cfg, "legacy_fixed_solar_orbs", False))
        if legacy_fixed:
            cazimi_orb = 17 / 60.0
            combustion_orb = 8.5
            under_beams_orb = 17.0
        else:
            cazimi_orb_arcmin = float(getattr(orbs_cfg, "cazimi_orb_arcmin", 17.0))
            cazimi_orb = cazimi_orb_arcmin / 60.0
            combustion_orb = float(getattr(orbs_cfg, "combustion_orb", 8.5))
            under_beams_orb = float(getattr(orbs_cfg, "under_beams_orb", 15.0))

            # Optional per-planet overrides:
            # orbs:
            #   per_planet_solar:
            #     Mars:
            #       combustion_orb: 8.0
            #       under_beams_orb: 14.0
            per_planet = getattr(orbs_cfg, "per_planet_solar", None)
            if per_planet is not None:
                planet_cfg = (
                    getattr(per_planet, planet.value, None)
                    or getattr(per_planet, planet.value.lower(), None)
                )
                if planet_cfg is not None:
                    combustion_orb = float(getattr(planet_cfg, "combustion_orb", combustion_orb))
                    under_beams_orb = float(getattr(planet_cfg, "under_beams_orb", under_beams_orb))
    ...
    def _calculate_comprehensive_traditional_dignity(self, planet: Planet, planet_pos: PlanetPosition,
        ...
        term and face.
        """
        # North Node doesn't have traditional dignities
...
class EnhancedTraditionalHoraryJudgmentEngine:
    ...
    def __init__(self):
        ...
        self.perfection_core = PerfectionCoreAPI()  # New unified perfection detection

    def _precompute_planet_state(self, chart: HoraryChart) -> None:
        """Precompute per-planet derived properties for consistent use across the engine.
        - daily_motion (alias of speed)
        - speed_class (very_slow/slow/average/fast/very_fast for Moon; slow/average/fast for others)
        - visibility copied from enhanced solar analysis if available
        """
        try:
            config = cfg()
        except Exception:
        ...
        for planet, pos in getattr(chart, 'planets', {}).items():
            if not pos:
                continue
            # Daily motion
            dm = abs(getattr(pos, 'speed', 0.0))
            setattr(pos, 'daily_motion', dm)

            # Speed class
            speed_class = 'average'
            if planet == Planet.MOON and config is not None:
                very_slow = getattr(config.timing, 'slow_moon_threshold', 11.0) - 1.0
                slow_thr = getattr(config.timing, 'slow_moon_threshold', 11.0)
                fast_thr = getattr(config.timing, 'fast_moon_threshold', 15.0)
                if dm < very_slow:
                ...
                elif dm >= (slow_thr + fast_thr) / 2:
                    speed_class = 'fast'
                else:
                    speed_class = 'average'
            else:
                # Generic thresholds for non-luminaries
                if dm < 0.05:
                    speed_class = 'slow'
                elif dm > 1.0:
                    speed_class = 'fast'
                    ...
                    speed_class = 'average'
            setattr(pos, 'speed_class', speed_class)

            # Visibility (if solar analyses stored on chart)
            if hasattr(chart, 'solar_analyses'):
    ...
    def judge_question(self, question: str, location: str, 
                      ...
                      include_internal_chart: bool = False,
                      ...
                      exaltation_confidence_boost: float = None,
                      house_system_code: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if latitude is not None and longitude is not None:
                lat = float(latitude)
                lon = float(longitude)
                full_location = str(location_name or location)
            else:
                # Fail-fast geocoding
                try:
            ...
            chart = self.calculator.calculate_chart(
                dt_local, dt_utc, timezone_used, lat, lon, full_location,
                house_system_code=house_system_code
            )
            # Precompute consistent per-planet state (speed class, visibility)
            ...
            category_rules = self._get_effective_category_rules(question_type, question_analysis)
            # UNIFIED FINALIZATION: Single path preserving all functional side-effects
            evaluation = _evaluate_enhanced(structured_reasoning, category_rules)
            judgment = self._finalize_judgment(judgment, evaluation, structured_reasoning, 
            ...
            try:
                ...
                # Never let UI sorting crash a judgment
                pass

            general_info = self._calculate_general_info(chart)
            considerations = self._calculate_considerations(
                chart,
                question_analysis,
            ...
            # Serialize chart data for frontend and include considerations for
            # downstream consumers that read chart_data only.
            chart_data_serialized = serialize_chart_for_frontend(chart, chart.solar_analyses)
            chart_data_serialized["considerations"] = considerations

            lost_object_location = None
            pet_analysis_for_location = question_analysis.get("pet_analysis") or {}
            if question_type == Category.LOST_OBJECT or (
                question_type == Category.PET and pet_analysis_for_location.get("family") == "missing"
            ):
                try:
                    sigs = self._identify_significators(chart, question_analysis)
                    querent_planet = sigs.get("querent")
                    quesited_planet = sigs.get("quesited")
                    if querent_planet and quesited_planet:
                        reception_info = (judgment.get("traditional_factors") or {}).get("reception")
                        if not isinstance(reception_info, dict):
                        ...
                        if question_type == Category.LOST_OBJECT:
                            lost_snapshot = build_lost_object_discovery_snapshot(
                                chart,
                                querent_planet,
                                quesited_planet,
                                question_analysis.get("lost_object_analysis"),
                                {"type": (judgment.get("traditional_factors") or {}).get("perfection_type")},
                                reception_info,
                            )
                            projection = build_lost_object_location_projection(lost_snapshot)
                        else:
                            pet_snapshot = build_pet_missing_snapshot(
                                chart,
                                quesited_planet,
                                pet_analysis_for_location,
                                {"type": (judgment.get("traditional_factors") or {}).get("perfection_type")},
                                reception_info,
            ...
            if lost_object_location:
                result["lost_object_location"] = lost_object_location
            if include_internal_chart:
                result["_raw_chart"] = chart
            return result
    ...
    def _calculate_considerations(
        ...
        """Return standard horary considerations with override context."""
        radicality = check_enhanced_radicality(chart, ignore_saturn_7th)
        moon_void = self._is_moon_void_of_course_enhanced(chart)

        if ignore_radicality:
            radical = True
            radical_reason = f"Bypassed by override ({radicality['reason']})"
        else:
        ...
        if ignore_void_moon:
            moon_void_reason = f"{moon_void_reason} (ignored for judgment gating)"

        return {
            "radical": radical,
            "radical_reason": radical_reason,
            "radical_raw": radicality["valid"],
            "radicality_ignored": bool(ignore_radicality),
            "moon_void": moon_void["void"],
            "moon_void_reason": moon_void_reason,
            "moon_void_ignored": bool(ignore_void_moon),
        }
    ...
    def _classify_question_intent(self, question: str, category: str) -> str:
        """Classify if a question asks for:
        - OCCURRENCE: 'will it happen?'
        - QUALITY   : 'is it favorable?'

        Unified version merging old/new heuristics and category defaults.
        """
        if not question:
            return "OCCURRENCE"  # conservative default
        ...
        safety_indicators = (
            'safe', 'safety', 'is she safe', 'is he safe', 'are they safe',
            'survive', 'survival', 'get better', 'recover', 'recovery',
            'healthy', 'well', 'improve', 'improving',
        )
        if any(k in q for k in affection_indicators):
            return "QUALITY"
        ...
        occurrence_indicators = (
            ...
            'going to happen','will happen','going to get','will get','will receive',
            'will find','will succeed','will fail','will be','going to be',
            'happen','occur','come to pass','succeed','work out','come back','return',
            'get','find','recover','obtain','achieve','likely to'
        )
        if any(k in q for k in occurrence_indicators):
            return "OCCURRENCE"

        # Category-based defaults for ambiguous questions
        # 1) Enum-based (preferred)
        try:
            from taxonomy import Category, resolve_category  # local import to avoid circulars
        ...
        if cat is not None:
            ...
            if cat in [Category.MARRIAGE, Category.CAREER, Category.FUNDING, Category.PROPERTY]:
                return "QUALITY"     # "Is this good for me?", "Should I take it?", etc.
            if cat == Category.TRAVEL:
                return "QUALITY" if any(w in q for w in ('should','good','wise','advisable')) else "OCCURRENCE"

        # 2) String fallback (some callers pass string categories)
        if isinstance(category, str):
            c = category.strip().upper()
            quality_cats = {'ADVICE','CHOICE','DECISION'}
            occurrence_cats = {'EVENT','OUTCOME','SUCCESS','FINDING','RETURN'}
            if c in quality_cats:
                return "QUALITY"
            if c in occurrence_cats:
                return "OCCURRENCE"

        # Final fallback
        return "OCCURRENCE"

    def _get_effective_category_rules(self, question_type, question_analysis: Dict[str, Any]) -> Dict[str, Any]:
        """Return category rules with property-family adjustments when needed."""
        category_rules = get_category_rules(question_type)
        if question_type == Category.PROPERTY:
            return derive_property_category_rules(
                category_rules, question_analysis.get("property_analysis")
            )
    ...
    def _apply_enhanced_judgment(self, chart: HoraryChart, question_analysis: Dict,
        ...
        relationship_analysis = question_analysis.get("relationship_analysis") or {}
        pregnancy_analysis = question_analysis.get("pregnancy_analysis") or {}
        health_analysis = question_analysis.get("health_analysis") or {}
        pet_analysis = question_analysis.get("pet_analysis") or {}
        lost_object_analysis = question_analysis.get("lost_object_analysis") or {}
        competition_analysis = question_analysis.get("competition_analysis") or {}
        if question_type == Category.RELATIONSHIP and relationship_analysis.get("family") == "durability":
            question_intent = "QUALITY"
        elif question_type == Category.PREGNANCY and pregnancy_analysis.get("family") == "diagnosis":
            question_intent = "QUALITY"
        elif question_type == Category.HEALTH and health_analysis.get("family") in {"diagnosis", "progression", "treatment"}:
            question_intent = "QUALITY"
        elif question_type == Category.PET and pet_analysis.get("family") == "recovery":
            question_intent = "SAFETY"
        # Expose intent for downstream consumers
        try:
            question_analysis["question_intent"] = question_intent
        except Exception:
            pass
        ...
        # Treat Moon as co-significator in analysis bundle
        sigs = question_analysis.setdefault("significators", {})
        cos = sigs.get("co_significators")
        if isinstance(cos, set):
            cos.add(Planet.MOON)
        else:
            sigs["co_significators"] = {Planet.MOON}

        # --- Global House 4 end-of-matters penalty (applies to all categories) ---
        # Add early so it's included in all reasoning paths
        malefics = {Planet.MARS, Planet.SATURN}
        for planet, pos in getattr(chart, "planets", {}).items():
            if not pos:
                continue
            if planet in malefics and getattr(pos, "house", None) == 4:
                # configurable weight if you like: cfg().weights.get("malefic_in_4th", -5)
                reasoning.append({
                    "stage": "House Conditions",
                    "key": "malefic_in_4th",
                    "rule": f"{planet.value} (malefic) in the 4th (end of matters)",
                    "weight": -5,
                    "house": 4,
                    "factor": "end_matter",
        ...
        if desc.startswith("Shared Significator") and significators.get("same_ruler_analysis"):
            ...
            # DEBUG: Print all relevant data to understand the issue
            print(f"\n=== DEBUG: Shared Significator Analysis ===")
            print(f"Initial description: {desc}")
            print(f"same_ruler_info: {same_ruler_info}")
            print(f"significators houses: {significators.get('houses')}")
            print(f"querent_house: {significators.get('querent_house')}")
            print(f"quesited_house: {significators.get('quesited_house')}")
            
            # Prefer house data from same_ruler_analysis but fall back as needed
            # CRITICAL FIX: If same_ruler_info houses are duplicated, use significators houses instead
            same_ruler_houses = same_ruler_info.get("houses")
            if same_ruler_houses and len(same_ruler_houses) >= 2 and same_ruler_houses[0] == same_ruler_houses[1]:
                print("same_ruler_info houses are duplicated, using significators houses instead")
                houses = list(significators.get("houses", []))
            else:
                houses = list(
                    same_ruler_houses
            ...
            print(f"Houses after list creation: {houses}")
            
            if len(houses) >= 2:
                print(f"Before deduplication check - houses[0]: {houses[0]}, houses[1]: {houses[1]}")
                
                # If houses are duplicated, use the distinct quesited house when available
                if houses[0] == houses[1]:
                    print("DEBUG: Houses are duplicated, attempting to fix...")
                    
                    # NEW APPROACH: If we have more than 2 houses, use the first and the first distinct one
                    if len(houses) > 2:
                        first_house = houses[0]
                        for i in range(1, len(houses)):
                            if houses[i] != first_house:
                                houses = [first_house, houses[i]]
                                print(f"SUCCESS (multi-house approach): Fixed houses to: {houses}")
                                break
                        else:
                            print("No distinct house found in the houses array")
                            # Fallback to original approach
                            q_house = significators.get("querent_house")
                            qd_house = significators.get("quesited_house")
                            print(f"Fallback approach - q_house: {q_house}, qd_house: {qd_house}")
                            
                            if q_house and qd_house and q_house != qd_house:
                                houses = [q_house, qd_house]
                                print(f"SUCCESS (fallback method): Fixed houses to: {houses}")
                            else:
                                print(f"FAILED: Could not fix - q_house={q_house}, qd_house={qd_house}, same={q_house == qd_house if q_house and qd_house else 'N/A'}")
                    else:
                        # Original approach for 2-house arrays
                        q_house = significators.get("querent_house")
                        qd_house = significators.get("quesited_house")
                        print(f"Original approach - q_house: {q_house}, qd_house: {qd_house}")
                        
                        if q_house and qd_house and q_house != qd_house:
                            houses = [q_house, qd_house]
                            print(f"SUCCESS (original method): Fixed houses to: {houses}")
                        else:
                            print(f"FAILED: Could not fix - q_house={q_house}, qd_house={qd_house}, same={q_house == qd_house if q_house and qd_house else 'N/A'}")
                else:
                    print("Houses are not duplicated")
                    
                print(f"Final houses: {houses}")
                
                # Update analysis with the deduplicated pair
                same_ruler_info["houses"] = houses
                shared = same_ruler_info.get("shared_ruler")
                if shared:
                    print(f"Creating new description with shared={shared.value}")
                    desc = (
        ...
        # Explicit negative testimony for retrograde quesited (UI-friendly, light weight)
        try:
            qsd_pos = chart.planets.get(quesited_planet)
            if getattr(qsd_pos, "retrograde", False):
                reasoning.append({
                    "stage": "Significators",
                    "rule": f"{quesited_planet.value} (quesited) is retrograde",
                    "weight": -1,
                })
        except Exception:
            pass
        ...
        if querent_prohibition["has_prohibition"]:
            for prohibition in querent_prohibition["prohibitions"]:
                # Stage by explicit kind rather than guessing from text
                if prohibition.get("kind") == "true_prohibition":
                    stage = "Prohibition"
                elif prohibition.get("kind") == "post_perfection":
                    stage = "Post-perfection corruption"
                else:
                    stage = "Affliction"
                reasoning.append({
                    "stage": stage,
        ...
        # NEW PERFECTION SYSTEM: Using unified perfection core
        new_result = self.perfection_core.find_perfections(chart, primary_significator, secondary_significator, window_days)
        perfection = new_result.get("primary") or {"perfects": False, "reason": "No perfection found", "type": "none"}
        if isinstance(perfection, dict) and new_result.get("secondary"):
            perfection["secondary"] = new_result.get("secondary")
        
        # PROCESS LOGGING: Show how new perfection system works (if enabled)
        # Enable with environment variable: HORARY_SHADOW_MODE=true  
        import os
        if os.getenv("HORARY_SHADOW_MODE", "false").lower() == "true":
            process_info = self._log_perfection_process(
                chart, primary_significator, secondary_significator, window_days, new_result
            )
            
            # Add process metadata to perfection result
            if isinstance(perfection, dict):
                perfection["process_logged"] = True
                perfection["process_info"] = process_info
        else:
            print("SHADOW MODE: Disabled (set HORARY_SHADOW_MODE=true to enable)")
        ...
        if question_type == Category.PREGNANCY and pregnancy_analysis.get("family") == "diagnosis":
            diagnosis_snapshot = build_pregnancy_diagnosis_snapshot(
                chart,
                querent_planet,
                quesited_planet,
                pregnancy_analysis,
            )
            diagnosis_eval = evaluate_pregnancy_diagnosis_snapshot(diagnosis_snapshot)
            if diagnosis_eval.get("applies"):
                reasoning.extend(diagnosis_eval.get("reasoning", []))
                return {
                    "result": diagnosis_eval["result"],
                    "confidence": diagnosis_eval.get("confidence", confidence),
                    "reasoning": reasoning,
                    "timing": None,
                    "traditional_factors": {
                        "perfection_type": "pregnancy_status_balance",
                        "pregnancy_family": "diagnosis",
        ...
        if question_type == Category.HEALTH and health_analysis.get("family") == "diagnosis":
            diagnosis_snapshot = build_health_diagnosis_snapshot(
                chart,
                querent_planet,
                quesited_planet,
                health_analysis,
            )
            diagnosis_eval = evaluate_health_diagnosis_snapshot(diagnosis_snapshot)
            if diagnosis_eval.get("applies"):
                reasoning.extend(diagnosis_eval.get("reasoning", []))
                return {
                    "result": diagnosis_eval["result"],
                    ...
                    "timing": None,
                    "traditional_factors": {
                        "perfection_type": "health_diagnosis_balance",
                        "health_family": "diagnosis",
                        "reception": self._detect_reception_between_planets(chart, querent_planet, quesited_planet),
                        "querent_strength": chart.planets[querent_planet].dignity_score,
                        "quesited_strength": chart.planets[quesited_planet].dignity_score,
                    },
        ...
        children_analysis = question_analysis.get("children_analysis") or {}
        if question_type == Category.CHILDREN and children_analysis.get("family") == "adoption":
            adoption_snapshot = build_children_adoption_snapshot(
                chart,
                quesited_planet,
                question_analysis,
                children_analysis,
            )
            adoption_eval = evaluate_children_adoption_snapshot(adoption_snapshot)
            if adoption_eval.get("applies"):
                reasoning.extend(adoption_eval.get("reasoning", []))
                return {
                    "result": adoption_eval["result"],
                    "confidence": adoption_eval.get("confidence", confidence),
                    "reasoning": reasoning,
                    "timing": None,
                    "traditional_factors": {
                        "perfection_type": "children_adoption_balance",
                        "children_family": "adoption",
                        "reception": self._detect_reception_between_planets(chart, querent_planet, quesited_planet),
                        "querent_strength": chart.planets[querent_planet].dignity_score,
                        "quesited_strength": chart.planets[quesited_planet].dignity_score,
                    },
        ...
        if (
            question_type == Category.HEALTH
            and health_analysis.get("family") == "progression"
            and not perfection["perfects"]
        ):
            progression_snapshot = build_health_progression_snapshot(
                chart,
                querent_planet,
                quesited_planet,
                health_analysis,
            )
            progression_eval = evaluate_health_progression_snapshot(progression_snapshot)
            if progression_eval.get("applies"):
                reasoning.extend(progression_eval.get("reasoning", []))
                return {
                    "result": progression_eval["result"],
                    "confidence": progression_eval.get("confidence", confidence),
                    "reasoning": reasoning,
                    "timing": None,
                    "traditional_factors": {
                        "perfection_type": "health_progression_balance",
                        "health_family": "progression",
                        "reception": self._detect_reception_between_planets(chart, querent_planet, quesited_planet),
                        "querent_strength": chart.planets[querent_planet].dignity_score,
                        "quesited_strength": chart.planets[quesited_planet].dignity_score,
                    },
        ...
        if (
            question_type == Category.RELATIONSHIP
            and relationship_analysis.get("family") == "durability"
            and not perfection["perfects"]
        ):
            reception_info = self.reception_calculator.calculate_comprehensive_reception(
                chart, querent_planet, quesited_planet
            )
            durability_snapshot = build_relationship_durability_snapshot(
                chart,
                querent_planet,
                quesited_planet,
            ...
            durability_eval = evaluate_relationship_durability_snapshot(durability_snapshot)
            if durability_eval.get("applies"):
                reasoning.extend(durability_eval.get("reasoning", []))
                return {
                    "result": durability_eval["result"],
                    "confidence": durability_eval.get("confidence", confidence),
                    "reasoning": reasoning,
                    "timing": None,
                    "traditional_factors": {
                        "perfection_type": "relationship_durability_balance",
                        "relationship_family": "durability",
        ...
        if (
            question_type == Category.PREGNANCY
            and pregnancy_analysis.get("family") == "conception"
            and not perfection["perfects"]
        ):
            conception_snapshot = build_pregnancy_conception_snapshot(
                chart,
                querent_planet,
                quesited_planet,
                pregnancy_analysis,
            )
            conception_eval = evaluate_pregnancy_conception_snapshot(conception_snapshot)
            if conception_eval.get("applies"):
                reasoning.extend(conception_eval.get("reasoning", []))
                return {
                    "result": conception_eval["result"],
                    "confidence": conception_eval.get("confidence", confidence),
                    "reasoning": reasoning,
                    "timing": moon_testimony.get("timing", "Moderate timeframe"),
                    "traditional_factors": {
                        "perfection_type": "pregnancy_sufficiency",
                        "pregnancy_family": "conception",
                        "reception": self._detect_reception_between_planets(chart, querent_planet, quesited_planet),
                        "querent_strength": chart.planets[querent_planet].dignity_score,
                        "quesited_strength": chart.planets[quesited_planet].dignity_score,
                    },
        ...
        if question_type == Category.PET and pet_analysis.get("family") == "missing":
            reception_info = self.reception_calculator.calculate_comprehensive_reception(
                chart, querent_planet, quesited_planet
            )
            pet_missing_snapshot = build_pet_missing_snapshot(
                chart,
                quesited_planet,
                pet_analysis,
                perfection,
            ...
            if pet_missing_eval.get("applies"):
                ...
                return {
                    "result": pet_missing_eval["result"],
                    "confidence": pet_missing_eval.get("confidence", confidence),
                    "reasoning": reasoning,
                    "timing": moon_testimony.get("timing", "Moderate timeframe"),
                    "traditional_factors": {
                        ...
                        "querent_strength": chart.planets[querent_planet].dignity_score,
                        "quesited_strength": chart.planets[quesited_planet].dignity_score,
                        "pet_safety_score": pet_safety.get("score", 0),
                        "moon_score": moon_testimony.get("score", 0),
                    },
                    "solar_factors": solar_factors,
                }
        ...
        if (
            ...
            and pet_analysis.get("family") == "recovery"
            and not perfection["perfects"]
        ):
            pet_recovery_snapshot = build_pet_recovery_snapshot(
                chart,
                quesited_planet,
                pet_analysis,
                pet_safety,
            ...
            if pet_recovery_eval.get("applies"):
                reasoning.extend(pet_recovery_eval.get("reasoning", []))
                return {
                    "result": pet_recovery_eval["result"],
                    "confidence": pet_recovery_eval.get("confidence", confidence),
                    "reasoning": reasoning,
                    "timing": moon_testimony.get("timing", "Moderate timeframe"),
                    "traditional_factors": {
                        "perfection_type": "pet_recovery_balance",
                        "pet_family": "recovery",
                        "reception": self._detect_reception_between_planets(chart, querent_planet, quesited_planet),
                        "querent_strength": chart.planets[querent_planet].dignity_score,
                        "quesited_strength": chart.planets[quesited_planet].dignity_score,
        ...
        if (
            ...
            and lost_object_analysis.get("family") in {"discovery", "discovery_timing"}
            and not perfection["perfects"]
        ):
            reception_info = self.reception_calculator.calculate_comprehensive_reception(
                chart, querent_planet, quesited_planet
            ...
            lost_eval = evaluate_lost_object_discovery_snapshot(lost_snapshot)
            if lost_eval.get("applies"):
                reasoning.extend(lost_eval.get("reasoning", []))
                return {
                    "result": lost_eval["result"],
                    "confidence": lost_eval.get("confidence", confidence),
                    "reasoning": reasoning,
                    "timing": None,
                    "traditional_factors": {
                        "perfection_type": "lost_object_discovery_balance",
                        "lost_object_family": lost_object_analysis.get("family"),
                        "reception": reception_info,
                        "querent_strength": chart.planets[querent_planet].dignity_score,
                        "quesited_strength": chart.planets[quesited_planet].dignity_score,
                    },
        ...
        if (
            competition_analysis.get("family") == "contest_public_office"
            and not perfection["perfects"]
        ):
            public_office_snapshot = build_public_office_contest_snapshot(
                chart,
                querent_planet,
                quesited_planet,
                competition_analysis,
            )
            public_office_eval = evaluate_public_office_contest_snapshot(public_office_snapshot)
            if public_office_eval.get("applies"):
                reasoning.extend(public_office_eval.get("reasoning", []))
                return {
                    "result": public_office_eval["result"],
                    "confidence": public_office_eval.get("confidence", confidence),
                    "reasoning": reasoning,
                    "timing": None,
                    "traditional_factors": {
                        "perfection_type": "competition_public_office_balance",
                        "competition_family": "contest_public_office",
                        "querent_strength": chart.planets[querent_planet].dignity_score,
                        "quesited_strength": chart.planets[quesited_planet].dignity_score,
                    },
                    "solar_factors": solar_factors,
        ...
        if (
            competition_analysis.get("family") == "contest_champion_vs_challenger"
            and not perfection["perfects"]
        ):
            champion_snapshot = build_champion_defense_snapshot(
                chart,
                querent_planet,
                quesited_planet,
                competition_analysis,
            )
            champion_eval = evaluate_champion_defense_snapshot(champion_snapshot)
            if champion_eval.get("applies"):
                reasoning.extend(champion_eval.get("reasoning", []))
                return {
                    "result": champion_eval["result"],
                    "confidence": champion_eval.get("confidence", confidence),
                    "reasoning": reasoning,
                    "timing": None,
                    "traditional_factors": {
                        "perfection_type": "competition_champion_balance",
                        "competition_family": "contest_champion_vs_challenger",
                        "querent_strength": chart.planets[querent_planet].dignity_score,
                        "quesited_strength": chart.planets[quesited_planet].dignity_score,
                    },
                    "solar_factors": solar_factors,
        ...
        if perfection["perfects"]:
            ...
            # CRITICAL: Set confidence from perfection, not from global base
            confidence = min(config.confidence.base_confidence, 
                           perfection.get("confidence", config.confidence.perfection.direct_basic))
            ...
            # New componentized messaging for direct perfections (presence, aspect, reception, retrograde)
            if perfection["type"] in ("direct", "direct_penalized"):
                # 1) Perfection presence (applying major aspect)
                if perfection.get("exact_in_days") is not None and perfection["exact_in_days"] > 0:
                    reasoning.append({
                        "stage": "Perfection",
                        "rule": "Perfection: Applying major aspect present",
                        "weight": 2,
                    })

                # 2) Aspect type scoring and message
                aspect_obj = perfection.get("aspect")
                aspect_name = getattr(aspect_obj, "display_name", "") or ""
                aspect_l = aspect_name.lower()
                aspect_weight = 0
                aspect_msg = None
                ...
                # 3) Reception scoring and message
                rec = perfection.get("reception") or {}
                rec_weight = 0
                rec_msg = None
                if isinstance(rec, dict):
                    mutual = rec.get("mutual", "none")
                    if mutual in ("mutual_rulership", "mutual_exaltation"):
                        rec_weight = 3
                        rec_msg = "Reception: Strong mutual (domicile/exaltation)"
                    elif mutual == "mixed_reception":
                        rec_weight = 1
                        rec_msg = "Reception: Mixed mutual (triplicity/term/face)"
                    else:
                        # One-way reception analysis via detailed lists
                        p1r = set(rec.get("planet1_receives_planet2", []) or [])
                        p2r = set(rec.get("planet2_receives_planet1", []) or [])
                        one_way = p1r.symmetric_difference(p2r)
                        def has_strong(digs):
                            return any(d in digs for d in ("domicile", "exaltation"))
                        def has_light(digs):
                            return any(d in digs for d in ("triplicity", "term", "face"))
                        if one_way:
                            if has_strong(one_way):
                                rec_weight = 2
                                rec_msg = "Reception: One-way (domicile/exaltation)"
                            elif has_light(one_way):
                                rec_weight = 1
                                rec_msg = "Reception: One-way (triplicity/term/face)"
                # Negative reception: received into detriment/fall (either direction)
                try:
                    detriment_signs = {
                        Planet.SUN: [Sign.AQUARIUS],
                        Planet.MOON: [Sign.CAPRICORN],
                    ...
                    q_sign = chart.planets[querent_planet].sign
                    lq_sign = chart.planets[quesited_planet].sign
                    neg_case = False
                    neg_detail = None
                    if lq_sign in detriment_signs.get(querent_planet, []) or self.falls.get(querent_planet) == lq_sign:
                        neg_case = True
                        neg_detail = f"{quesited_planet.value} in {lq_sign.sign_name} is detriment/fall of {querent_planet.value}"
                    if q_sign in detriment_signs.get(quesited_planet, []) or self.falls.get(quesited_planet) == q_sign:
                        neg_case = True
                        neg_detail = f"{querent_planet.value} in {q_sign.sign_name} is detriment/fall of {quesited_planet.value}"
                    if neg_case:
                        reasoning.append({
                            "stage": "Perfection",
                    ...
                    pass
                if rec_msg:
                    reasoning.append({"stage": "Perfection", "rule": rec_msg, "weight": rec_weight})

                # 4) Retrograde at application (approximate by current state)
                q_pos = chart.planets.get(querent_planet)
                qq_pos = chart.planets.get(quesited_planet)
                if getattr(q_pos, "retrograde", False) or getattr(qq_pos, "retrograde", False):
                    reasoning.append({
                        "stage": "Perfection",
                        "rule": "Retrograde at application (either significator)",
                        "weight": -1,
                    })
            ...
            if perfection["type"] == "direct_penalized":
                aspect_obj = perfection.get("aspect")
                aspect_name = (getattr(aspect_obj, "display_name", "") or "").lower()
                is_easy = ("sextile" in aspect_name or "trine" in aspect_name or "conjunction" in aspect_name)
                is_hard = ("square" in aspect_name or "opposition" in aspect_name)

                # Improve messaging and scoring to reflect quality:
                # - Easy aspects: supportive but "with obstacles"
                # - Hard aspects: occurrence present, but opposes favorable verdict unless strong reception
                if is_easy:
                    rule_text = "Perfection with obstacles"
                    weight = 4
                elif is_hard:
                    ...
                    rec = perfection.get("reception") or {}
                    mutual = (rec.get("mutual") if isinstance(rec, dict) else None) or "none"
                    if mutual == "mixed_reception" and weight < 0:
                        weight = -2
                else:
                    # Fallback text for other penalized cases
                    rule_text = "Perfection with obstacles"
                    weight = 2

                reasoning.append({"stage": "Perfection", "rule": rule_text, "weight": weight})
            ...
            relationship_analysis = question_analysis.get("relationship_analysis") or {}
            if question_type == Category.RELATIONSHIP and relationship_analysis.get("family") == "affection":
                affection_snapshot = build_relationship_affection_snapshot(
                    chart,
                    querent_planet,
                    quesited_planet,
                    relationship_analysis,
                    perfection,
                    perfection.get("reception") if isinstance(perfection.get("reception"), dict) else None,
                )
                affection_eval = evaluate_relationship_affection_snapshot(affection_snapshot)
                if affection_eval.get("applies"):
                    reasoning.extend(affection_eval.get("reasoning", []))
                    return {
                        "result": affection_eval["result"],
                        "confidence": affection_eval.get("confidence", confidence),
                        "reasoning": reasoning,
                        "timing": self._calculate_enhanced_timing(chart, perfection),
                        "traditional_factors": {
                            "perfection_type": "relationship_affection_balance",
                            "relationship_family": "affection",
                            "reception": self._detect_reception_between_planets(chart, querent_planet, quesited_planet),
                            "querent_strength": chart.planets[querent_planet].dignity_score,
                            "quesited_strength": chart.planets[quesited_planet].dignity_score,
                        },
            ...
            economic_analysis = question_analysis.get("economic_analysis") or {}
            if economic_analysis.get("family") == "bank_counterparty":
                reception_info = self.reception_calculator.calculate_comprehensive_reception(
                    chart, querent_planet, quesited_planet
                )
                bank_snapshot = build_bank_counterparty_snapshot(
                    chart,
                    querent_planet,
                    quesited_planet,
                    economic_analysis,
                ...
                bank_eval = evaluate_bank_counterparty_snapshot(bank_snapshot)
                if bank_eval.get("applies"):
                    reasoning.extend(bank_eval.get("reasoning", []))
                    return {
                        "result": bank_eval["result"],
                        "confidence": bank_eval.get("confidence", confidence),
                        "reasoning": reasoning,
                        "timing": self._calculate_enhanced_timing(chart, perfection),
                        "traditional_factors": {
                            "perfection_type": "bank_counterparty_settlement",
                            "economic_family": "bank_counterparty",
                            "reception": reception_info,
                            "querent_strength": chart.planets[querent_planet].dignity_score,
                            "quesited_strength": chart.planets[quesited_planet].dignity_score,
                        },
        ...
        supportive_signals = []
        supportive_reasoning: List[Dict[str, Any]] = []
        ...
        if mutual != "none":
            ...
            supportive_reasoning.append({
                "stage": "Secondary Testimony",
                "rule": f"Mutual reception between {querent_planet.value} and {quesited_planet.value}: {mutual}",
                "weight": reception_bonus,
                "factor": "reception",
            })
        elif one_way:
            reception_bonus = getattr(
            ...
            supportive_reasoning.append({
                "stage": "Secondary Testimony",
                "rule": f"One-way reception between {querent_planet.value} and {quesited_planet.value}",
                "weight": reception_bonus,
                "factor": "reception",
            })
        ...
        if moon_next_aspect and moon_next_aspect.applying:
            ...
            if target in [querent_planet, quesited_planet]:
                if moon_next_aspect.aspect in [Aspect.TRINE, Aspect.SEXTILE]:
                    ...
                    supportive_reasoning.append({
                        "stage": "Moon Testimony",
                        "rule": moon_display,
                        "weight": moon_bonus,
                        "factor": "moon_testimony",
                    })
        ...
        property_quality_eval = None
        if question_type == Category.PROPERTY and not occurrence_focused:
            property_snapshot = build_property_advisability_snapshot(
                chart,
                querent_planet,
                question_analysis.get("property_analysis"),
            )
            property_quality_eval = evaluate_property_advisability_snapshot(property_snapshot)
            if property_quality_eval.get("applies"):
                supportive_reasoning.extend(property_quality_eval.get("reasoning", []))

                family = (question_analysis.get("property_analysis") or {}).get("family")
                if family == "advisability_profit":
                    if property_quality_eval.get("profit_score", 0) > 0:
                        supportive_signals.append(
                            f"10th-house profit testimony (+{property_quality_eval['profit_score']})"
                        )
                    if property_quality_eval.get("property_score", 0) > 0:
                        supportive_signals.append(
                            f"4th-house property testimony (+{property_quality_eval['property_score']})"
                        )
                    elif property_quality_eval.get("property_score", 0) < 0:
                        supportive_signals.append(
                            f"4th-house property weakness ({property_quality_eval['property_score']})"
                        )

                if property_quality_eval.get("result") in {"YES", "NO"}:
                    reasoning.extend(supportive_reasoning)
                    return {
                        "result": property_quality_eval["result"],
                        "confidence": property_quality_eval.get("confidence", 65),
                        "reasoning": reasoning,
                        "timing": None,
                        "traditional_factors": {
                            "perfection_type": "property_quality_sufficiency",
                            "property_family": family,
                            "property_score": property_quality_eval.get("score", 0),
                            "profit_score": property_quality_eval.get("profit_score", 0),
                            "end_score": property_quality_eval.get("property_score", 0),
                            "querent_strength": chart.planets[querent_planet].dignity_score,
                            "quesited_strength": chart.planets[quesited_planet].dignity_score,
                            "reception": self._detect_reception_between_planets(chart, querent_planet, quesited_planet),
                        },
        ...
        secondary_balance_eval = self._evaluate_generic_secondary_balance(
            chart,
            querent_planet,
            quesited_planet,
            question_type,
            ...
            moon_next_aspect_result,
            benefic_support,
            benefic_support_overridden,
            reception_info,
        )

        if secondary_balance_eval.get("applies"):
            reasoning.extend(secondary_balance_eval.get("reasoning", []))
            reasoning.extend(supportive_reasoning)

            final_confidence = self._apply_debilitation_and_cadent_penalties(
                secondary_balance_eval.get("confidence", confidence),
                chart,
                querent_planet,
                quesited_planet,
                reasoning,
            ...
            return {
                "result": secondary_balance_eval["result"],
                "confidence": int(final_confidence),
                "reasoning": reasoning,
                "timing": None,
                "traditional_factors": {
                    "perfection_type": secondary_balance_eval["bucket"],
                    "secondary_balance_score": secondary_balance_eval.get("score", 0),
                    "secondary_balance_positive": secondary_balance_eval.get("positive_score", 0),
                    "secondary_balance_negative": secondary_balance_eval.get("negative_score", 0),
                    "querent_strength": chart.planets[querent_planet].dignity_score,
                    "quesited_strength": chart.planets[quesited_planet].dignity_score,
                    "reception": self._get_reception_for_structured_output(chart, querent_planet, quesited_planet),
        ...
        if occurrence_focused:
            ...
            reasoning.append({
                ...
                "rule": (
                    f"No direct perfection found between {querent_planet.value} and {quesited_planet.value}; "
                    "judging the question by condition and secondary testimonies"
                ),
                "weight": 0,
                "factor": "quality",
            })
        ...
        return {
            ...
            "traditional_factors": {
                ...
                "benefic_noted": benefic_support.get("total_score", 0) > 0,
                **(
                    {"property_family": (question_analysis.get("property_analysis") or {}).get("family")}
                    if question_type == Category.PROPERTY
                    else {}
                ),
            },
    ...
    def _evaluate_generic_secondary_balance(
        self,
        chart: HoraryChart,
        querent_planet: Planet,
        quesited_planet: Planet,
        question_type: Any,
        occurrence_focused: bool,
        moon_testimony: Dict[str, Any],
        moon_next_aspect_result: Dict[str, Any],
        benefic_support: Dict[str, Any],
        benefic_support_overridden: bool,
        reception_info: Dict[str, Any],
    ) -> Dict[str, Any]:
        """Evaluate qualified secondary testimony for no-route occurrence charts."""

        resolved_category = resolve_category(question_type)
        excluded_categories = {
        ...
        if not occurrence_focused or resolved_category in excluded_categories:
        ...
        connecting_testimony = False
        strong_connecting_testimony = False

        reception_strength = int(reception_info.get("traditional_strength", 0) or 0)
        reception_type = reception_info.get("type", "none")
        mutual_reception = reception_info.get("mutual", "none")
        if mutual_reception != "none" and reception_strength >= 3:
            positive_score += 4
            major_supports += 1
            reasoning.append(
                {
                    "stage": "Secondary Balance",
                    "rule": f"Mutual reception remains meaningful even without a direct route: {reception_type}",
                    "weight": 4,
                    "factor": "reception",
                }
            )
        elif reception_strength >= 8:
            positive_score += 5
            major_supports += 1
            reasoning.append(
                {
                    "stage": "Secondary Balance",
                    "rule": f"Substantial reception between {querent_planet.value} and {quesited_planet.value}: {reception_type}",
                    "weight": 5,
                    "factor": "reception",
                }
            )
        elif reception_strength >= 5:
            positive_score += 4
            major_supports += 1
            reasoning.append(
                {
                    "stage": "Secondary Balance",
                    "rule": f"Material reception between {querent_planet.value} and {quesited_planet.value}: {reception_type}",
                    "weight": 4,
                    "factor": "reception",
                }
            )
        elif reception_strength >= 3:
            positive_score += 2
            reasoning.append(
                {
                    "stage": "Secondary Balance",
                    "rule": f"Limited reception remains present without perfection: {reception_type}",
                    "weight": 2,
                    "factor": "reception",
                }
            )

        moon_result = moon_next_aspect_result.get("result")
        moon_reason = moon_next_aspect_result.get("reason")
        if moon_result == "YES":
            moon_bonus = 5 if moon_next_aspect_result.get("decisive") else 3
            positive_score += moon_bonus
            if moon_bonus >= 5:
                major_supports += 1
            connecting_testimony = True
            if moon_next_aspect_result.get("decisive"):
                strong_connecting_testimony = True
            reasoning.append(
                {
                    "stage": "Secondary Balance",
                    "rule": f"Moon next aspect remains favorable without direct perfection: {moon_reason}",
                    "weight": moon_bonus,
                    "factor": "moon_testimony",
                }
            )
        elif moon_result == "UNCLEAR":
            positive_score += 3
            reasoning.append(
                {
                    "stage": "Secondary Balance",
                    "rule": f"Moon next aspect softens denial even without a full route: {moon_reason}",
                    "weight": 3,
                    "factor": "moon_testimony",
                }
            )
        elif moon_result == "NO" and moon_next_aspect_result.get("decisive"):
            negative_score += 2
            reasoning.append(
                {
                    "stage": "Secondary Balance",
                    "rule": f"Moon next aspect remains adverse: {moon_reason}",
                    "weight": -2,
                    "factor": "moon_testimony",
                }
            )

        if not moon_testimony.get("void_of_course", False):
            positive_score += 1
            reasoning.append(
                {
                    "stage": "Secondary Balance",
                    "rule": "Moon not void of course keeps the matter active",
                    "weight": 1,
                    "factor": "moon_condition",
                }
            )

        benefic_score = int(benefic_support.get("total_score", 0) or 0)
        if benefic_support.get("favorable") and not benefic_support_overridden:
            if benefic_score >= 15:
                positive_score += 4
                major_supports += 1
                benefic_weight = 4
            elif benefic_score >= 8:
                positive_score += 3
                benefic_weight = 3
            elif benefic_score > 0:
                positive_score += 1
                benefic_weight = 1
            else:
                benefic_weight = 0
            ...
            if benefic_weight:
                reasoning.append(
                    {
                        "stage": "Secondary Balance",
                        "rule": f"Strong benefic help remains present: {benefic_support.get('reason', 'benefic testimony')}",
                        "weight": benefic_weight,
                        "factor": "benefic_support",
                    }
                )
        elif benefic_support_overridden:
            negative_score += 1

        quesited_position = chart.planets[quesited_planet]
        solar_analysis = (chart.solar_analyses or {}).get(quesited_planet)
        solar_condition = getattr(getattr(solar_analysis, "condition", None), "condition_name", "")
        workable_quesited = False

        if (
            quesited_position.dignity_score >= 4
            and not quesited_position.retrograde
            and solar_condition not in {SolarCondition.COMBUSTION.condition_name, SolarCondition.UNDER_BEAMS.condition_name}
        ):
            positive_score += 3
            major_supports += 1
            workable_quesited = True
            reasoning.append(
                {
                    "stage": "Secondary Balance",
                    "rule": f"{quesited_planet.value} remains in workable condition for the matter",
                    "weight": 3,
                    "factor": "quesited_condition",
                }
            )
        elif (
            quesited_position.dignity_score >= 0
            and not quesited_position.retrograde
            and solar_condition != SolarCondition.COMBUSTION.condition_name
        ):
            positive_score += 2
            workable_quesited = True
            reasoning.append(
                {
                    "stage": "Secondary Balance",
                    "rule": f"{quesited_planet.value} is not severely damaged despite the missing route",
                    "weight": 2,
                    "factor": "quesited_condition",
                }
            )

        if quesited_position.dignity_score <= -4:
            negative_score += 3
        if quesited_position.retrograde:
            negative_score += 2
        if solar_condition == SolarCondition.COMBUSTION.condition_name:
            negative_score += 3
        elif solar_condition == SolarCondition.UNDER_BEAMS.condition_name:
            negative_score += 1

        if reception_strength >= 5 and moon_result in {"YES", "UNCLEAR"}:
            positive_score += 2
            major_supports += 1
            reasoning.append(
                {
                    "stage": "Secondary Balance",
                    "rule": "Reception and Moon testimony reinforce one another despite the missing direct route",
                    "weight": 2,
                    "factor": "secondary_synergy",
                }
            )

        if workable_quesited and benefic_score >= 8 and not benefic_support_overridden:
            positive_score += 1
        ...
        if (
            ...
            and strong_connecting_testimony
        ):
            bucket = "affirmative_secondary_balance"
            result = "YES"
            confidence = min(70, 58 + net_score)
            summary_rule = (
                f"No direct perfection found between {querent_planet.value} and {quesited_planet.value}, "
                "but a strong connecting testimony keeps the matter affirmative despite the missing direct route"
            )
        elif (major_supports >= 2 and net_score >= 6) or (
            positive_score >= 8 and negative_score <= 2
        ):
            bucket = "mixed_or_inconclusive_secondary_balance"
            result = "UNCLEAR"
            confidence = min(62, 48 + net_score)
            if connecting_testimony:
                summary_rule = (
                    f"No direct perfection found between {querent_planet.value} and {quesited_planet.value}; "
                    "secondary testimony is substantial, but the bridge is not strong enough for a clean affirmative judgment"
                )
            else:
                summary_rule = (
                    f"No direct perfection found between {querent_planet.value} and {quesited_planet.value}; "
                    "secondary testimony suggests viability, but lacks a strong traditional bridge"
                )
        else:
            bucket = "denial_secondary_balance"
            result = "NO"
            confidence = max(58, min(78, 60 + (negative_score * 4) - min(positive_score, 4)))
            summary_rule = (
                f"No direct perfection found between {querent_planet.value} and {quesited_planet.value}; "
                "secondary testimony remains insufficient to overcome denial"
            )
        ...
        reasoning.insert(
            0,
            {
                ...
                "rule": summary_rule,
                "weight": net_score,
                "factor": "generic_secondary_balance",
            },
        )
    ...
    def _apply_retrograde_quesited_penalty(self, confidence: float, chart: HoraryChart,
        ...
        if quesited_pos.retrograde:
            ...
            # Emit structured entry for consistent frontend rendering (stage, weight)
            reasoning.append({
                "stage": "Significators",
                "rule": "Retrograde quesited (turning away from success)",
                "weight": -penalty,
            })
        ...
        for house in outcome_houses:
            ...
            if pos.dignity_score <= threshold:
                ...
                if penalty:
                    ...
                    reasoning.append({
                        "stage": "Impediment",
                        "rule": f"Debilitated L{house} ruler ({ruler.value})",
                        "weight": -penalty,
                    })
            ...
            for planet in [querent, quesited]:
                ...
                if pos:
                    ...
                    if angularity == "cadent":
                        ...
                        reasoning.append({
                            "stage": "Impediment",
                            "rule": f"{planet.value} in cadent house",
                            "weight": -cadent_penalty,
                        })
    ...
    def _is_moon_void_of_course_enhanced(self, chart: HoraryChart) -> Dict[str, Any]:
        ...
        # DEBUG: Compare different aspect calculation systems for Moon
        self._debug_compare_moon_aspect_systems(chart)
    ...
    def _void_traditional_ground_truth(self, chart: HoraryChart) -> Dict[str, Any]:
        ...
        # Optional diagnostics for the VOC decision path.
        ...
        if days_until_sign_exit is None:
        ...
        _debug_log(logging.INFO, f"[VOC] Days until sign exit: {days_until_sign_exit:.4f}")

        # Check if Moon is stationary (cannot be void if not moving)
        if abs(moon_pos.speed) < config.timing.stationary_speed_threshold:
            _debug_log(
                logging.INFO,
                f"[VOC] Moon is stationary (speed {abs(moon_pos.speed):.4f} < threshold {config.timing.stationary_speed_threshold}) - not void",
            )
            return {
                "void": False,
                "exception": False,
                "reason": "Moon stationary - cannot be void of course",
                "degrees_left_in_sign": degrees_to_sign_exit,
                "first_applying_aspect": None,
            }

        # Determine the first IN-SIGN lunar perfection analytically (Lilly-style)
        from .aspects import calculate_moon_next_aspect

        _debug_log(logging.INFO, "[VOC] Using enhanced aspects: calculate_moon_next_aspect(ignore_orb_for_voc=True)")
        moon_next_aspect = calculate_moon_next_aspect(
            chart.planets, chart.julian_day, ignore_orb_for_voc=True
        )

        # The helper already enforces in-sign perfection; just read the ETA.
        eta = getattr(moon_next_aspect, "perfection_eta_days", None) if moon_next_aspect else None
        cross_sign = False  # by construction (helper filters out cross-sign)
        
        if moon_next_aspect:
            _debug_log(
                logging.INFO,
                f"[VOC] Found applying aspect: {moon_next_aspect.aspect.display_name} to {moon_next_aspect.planet.value} in {eta:.4f} days",
            )
        else:
            _debug_log(logging.INFO, "[VOC] No applying aspects found within current sign")

        if moon_next_aspect is None or eta is None:
            ...
            _debug_log(logging.INFO, "[VOC] DECISION: Moon is VOID OF COURSE")
            _debug_log(logging.INFO, f"[VOC] Sign mitigation: {sign_mitigation} (Moon in {moon_pos.sign.sign_name})")
            ...
            return {
                ...
                "degrees_left_in_sign": degrees_to_sign_exit,
                "days_until_sign_exit": days_until_sign_exit,
                "first_applying_aspect": None,
        ...
        _debug_log(
            logging.INFO,
            f"[VOC] DECISION: Moon is NOT void of course - will {moon_next_aspect.aspect.display_name.lower()} {moon_next_aspect.planet.value}",
        )
        return {
            "void": False,
            "exception": False,
    ...
    # REMOVED: standard aspect helper eliminated (use aspects.calculate_moon_next_aspect)
    
    def _debug_compare_moon_aspect_systems(self, chart: HoraryChart):
        """DEBUG: Compare Enhanced Moon aspects vs VOC determinations (standard system removed)"""
        
        _debug_log(logging.INFO, "[ASPECT-DEBUG] ===== COMPARING MOON ASPECT SYSTEMS =====")
        
        import math
        from .aspects import calculate_enhanced_aspects, calculate_moon_next_aspect, time_to_perfection
        from .aspects import _will_perfect_before_sign_exit, _is_orb_shrinking
        ...
        moon_enhanced = [a for a in enhanced_aspects if Planet.MOON in [a.planet1, a.planet2]]

        _debug_log(logging.INFO, "[ASPECT-DEBUG] === SYSTEM 1: Enhanced Aspects (Moon-only) ===")
        if moon_enhanced:
            for a in moon_enhanced:
                _debug_log(
                    logging.INFO,
                    f"[ASPECT-DEBUG] {a.aspect.display_name} with "
                    f"{a.planet2.value if a.planet1 == Planet.MOON else a.planet1.value}: "
                    f"applying={a.applying}, within_sign={a.perfection_within_sign}, "
                    f"t={a.time_to_perfection:.4f}d"
                )
        else:
            _debug_log(logging.INFO, "[ASPECT-DEBUG] No Moon aspects found in enhanced list")

        # 2) VOC-mode (enhanced Moon-next, ignore orb)
        _debug_log(logging.INFO, "[ASPECT-DEBUG] === SYSTEM 2: VOC Mode (ignore_orb_for_voc=True) ===")
        moon_next_voc = calculate_moon_next_aspect(chart.planets, chart.julian_day, ignore_orb_for_voc=True)
        if moon_next_voc:
            _debug_log(
                logging.INFO,
                f"[ASPECT-DEBUG] Next Moon perfection: {moon_next_voc.aspect.display_name} "
                f"to {moon_next_voc.planet.value}, eta={moon_next_voc.perfection_eta_days:.4f}d"
            )
        else:
            _debug_log(logging.INFO, "[ASPECT-DEBUG] No next aspect found (VOC mode)")
        
        # 4) Detailed comparison
        _debug_log(logging.INFO, "[ASPECT-DEBUG] === DETAILED PLANET-BY-PLANET COMPARISON ===")
        ...
        for planet in classical_planets:
            ...
            for aspect_type in Aspect:
                ...
                # Method C: Orb shrinking (Enhanced method)
                applying_orb = _is_orb_shrinking(moon_pos, planet_pos, aspect_type)

                within_sign = _will_perfect_before_sign_exit(moon_pos, planet_pos, aspect_type, t_aspects) if t_aspects > 0 else False

                _debug_log(
                    logging.INFO,
                    f"[ASPECT-DEBUG]     {aspect_type.display_name}: "
                    f"aspects_t={t_aspects:.4f}d, orb_shrink={applying_orb}, "
                    f"within_sign={within_sign}"
                )
        
        # 5) Summary: do Enhanced vs VOC agree on the next Moon perfection within sign?
        try:
            enhanced_next = min(
                (a for a in moon_enhanced if a.applying and a.perfection_within_sign),
                key=lambda a: a.time_to_perfection,
                default=None,
            )
            if moon_next_voc and enhanced_next:
                same_aspect = (enhanced_next.aspect == moon_next_voc.aspect)
                same_target = ((enhanced_next.planet1 == Planet.MOON and enhanced_next.planet2 == moon_next_voc.planet) or
                               (enhanced_next.planet2 == Planet.MOON and enhanced_next.planet1 == moon_next_voc.planet))
                time_close = abs(enhanced_next.time_to_perfection - moon_next_voc.perfection_eta_days) < 1e-3
                status = "AGREE" if (same_aspect and same_target and time_close) else "DISAGREE"
                _debug_log(logging.INFO, f"[ASPECT-DEBUG] SUMMARY: Enhanced vs VOC => {status}")
            elif not moon_next_voc:
                _debug_log(logging.INFO, "[ASPECT-DEBUG] SUMMARY: VOC reports Moon void (no next aspect within sign).")
            else:
                _debug_log(logging.INFO, "[ASPECT-DEBUG] SUMMARY: Enhanced has no Moon applying within sign; VOC found one.")
        except Exception as e:
            _debug_log(logging.WARNING, f"[ASPECT-DEBUG] SUMMARY step failed: {e}")
        
        _debug_log(logging.INFO, "[ASPECT-DEBUG] ===== END MOON ASPECT SYSTEMS COMPARISON =====")
    
    # REMOVED: legacy VOC timing helper
    ...
    def _check_future_prohibitions(self, chart: HoraryChart, querent: Planet, quesited: Planet,
        ...
        Uses the unified perfection_core to detect prohibition, translation and collection
        events within the specified timeframe and converts them to engine format.
        """
        
        # Use days_ahead as the window for core detection
        window_days = max(int(days_ahead + 1), 7)  # Minimum 7 days to catch events
        result = self.perfection_core.find_perfections(chart, querent, quesited, window_days)
        
        all_events = result.get("events", [])
        
        # Filter events that occur within days_ahead timeframe
        relevant_events = []
        for event in all_events:
            if hasattr(event, 'exact_in_days') and event.exact_in_days is not None:
                if 0 < event.exact_in_days <= days_ahead:
        ...
        # Sort by timing to get earliest event
        relevant_events.sort(key=lambda e: e.exact_in_days or float('inf'))
        earliest_event = relevant_events[0]
        
        # Convert core event to engine format
        return self._convert_core_event_to_prohibition_format(earliest_event)
        
    def _convert_core_event_to_prohibition_format(self, event) -> Dict[str, Any]:
        """Convert perfection_core event to _check_future_prohibitions format"""
        from .perfection_core import EventType
        
        event_type = event.event_type
        timing = event.exact_in_days
        reason = event.reason
        
        if event_type == EventType.PROHIBITION:
            # forward richer context for UI/debugging
            return {
                "prohibited": True,
                "type": "prohibition", 
                "reason": reason,
                "prohibitor": event.mediator,
                "significator": event.metadata.get("target") if hasattr(event, "metadata") else (event.primary_pair[1] if event.mediator else None),
                "aspect": event.aspect,
                "t_event": timing,
                "target_house": (event.metadata or {}).get("target_house"),
                "prohibitor_house": (event.metadata or {}).get("prohibitor_house"),
                "preempts_in_days": (event.metadata or {}).get("preempts_in_days"),
            }
        elif event_type in [EventType.REFRANATION, EventType.FRUSTRATION, EventType.ABSCISSION]:
            return {
                "prohibited": True,
            ...
            return {
                ...
                "reason": reason,
                "t_event": timing,
                "translator": event.mediator,
                "aspect": event.aspect,
                "reception": event.reception_data.get("type", "none") if event.reception_data else "none",
                "quality": "favorable" if event.favorable else "unfavorable"
            }
        elif event_type == EventType.COLLECTION:
            return {
                "type": "collection", 
                "reason": reason,
                "t_event": timing,
                "collector": event.mediator,
                "aspect": event.aspect,
                "reception": event.reception_data.get("type", "none") if event.reception_data else "none",
                "quality": "favorable" if event.favorable else "unfavorable"
            }
        else:
            # For direct/house_placement or other events, return empty (no prohibition)
            return {}
    ...
    # (Removed duplicate _classify_question_intent; unified version appears earlier.)
    ...
    def _finalize_judgment(self, judgment: Dict, evaluation: Dict, structured_reasoning: List, 
                          chart: HoraryChart, question_analysis: Dict, question_type, category_rules: Dict) -> Dict:
        """UNIFIED finalization path preserving ALL functional side-effects from both branches"""
        
        # Helper function for confidence clamping
        def _clamp_conf(x, lo=10, hi=90):
            return max(lo, min(hi, float(x)))
        
        # BRANCH 1: Hybrid confidence calculation (COMPLETE PRESERVATION)
        if judgment.get("hybrid_confidence_needed"):
            ...
            if not blockers and judgment["traditional_factors"].get("perfection_type") == "none":
                ...
                elif question_intent == "QUALITY":
                    # Quality questions may still judge favorability without a perfection route.
                    # Do not synthesize a blocker that automatically reinforces a NO verdict.
                    blockers = []
                    soften_no_perfection = True
                else:
            ...
            # CRITICAL DEBUG: Log all inputs to hybrid confidence calculation (PRESERVED)
            ...
            # CRITICAL FIX: Use lower base confidence for SAFETY mode questions (PRESERVED)
            if question_intent == "SAFETY":
        ...
        # COMMON: Always set reasoning (PRESERVED)
        judgment["reasoning"] = structured_reasoning

        # BRANCH 2: Perfection preservation path (COMPLETE PRESERVATION)
        ...
        # Determine if we have a valid perfection type (PRESERVED)
        tf = judgment.get("traditional_factors", {}) or {}
        perfection_type = tf.get("perfection_type")
        valid_perfection_types = {
            "direct",
            "direct_penalized",  # FIXED: Preserve engine confidence for penalized direct perfections
            "translation",
            "collection",
            "house_placement",  # NEW: House placement perfections
            "same_ruler_unity",
            ...
            "pregnancy_status_balance",
            "property_quality_sufficiency",
            "relationship_affection_balance",
            "relationship_durability_balance",
            "children_adoption_balance",
            "pet_recovery_balance",
            "pet_missing_balance",
            "lost_object_discovery_balance",
            "competition_public_office_balance",
            "competition_champion_balance",
            "affirmative_secondary_balance",
            "mixed_or_inconclusive_secondary_balance",
            "denial_secondary_balance",
        }
        
        # Valid blocking/denial factors that should also use engine/hybrid confidence (PRESERVED)
        ...
        if not preserve_separated_confidence:
            if judgment.get("hybrid_confidence_needed"):
                # Already set by hybrid calc above (PRESERVED)
                pass
            elif perfection_type in valid_perfection_types or perfection_type in valid_blocking_types:
                # Keep engine-derived confidence for valid perfection OR blocking factors (PRESERVED)
                ...
                if isinstance(engine_confidence, (int, float)) and engine_confidence > 0:
                    ...
                    # CRITICAL FIX: For blocking factors, still use hybrid confidence instead of evaluation (PRESERVED)
                    if perfection_type in valid_blocking_types:
                        ...
                        # Use evaluation confidence for missing perfection confidence (PRESERVED)
                ...
                # Fall back to evaluation-derived confidence when no perfection (PRESERVED)
                judgment["confidence"] = int(evaluation["confidence"])
                judgment["confidence_source"] = "evaluation"

        # COMMON: Always set scoring trace (PRESERVED)
        judgment["scoring_trace"] = evaluation["trace"]
        
        # Apply confidence clamping to avoid extremes
        judgment["confidence"] = int(_clamp_conf(judgment["confidence"]))
        
        return judgment
    
    def _log_perfection_process(self, chart: HoraryChart, querent: Planet, quesited: Planet,
                               window_days: int, new_result: Dict[str, Any]) -> Dict[str, Any]:
        """
        PROCESS LOGGING: Show detailed information about how the new perfection system works.
        This logs the complete detection process for analysis and debugging.
        
        Args:
            chart: The horary chart
            querent: Primary significator
            quesited: Secondary significator  
            window_days: Time window for perfections
            new_result: Results from the new perfection system
        ...
        process_info = {
            "process_logged": True,
            "system_used": "unified_perfection_core",
            "events_detected": len(new_result.get("events", [])),
            "window_days": window_days
        }
        
        print(f"\n=== PERFECTION PROCESS: Unified Detection System ===")
        print(f"Analyzing: {querent.value} (querent) -> {quesited.value} (quesited)")
        print(f"Time window: {window_days} days")
        
        # Show all detected events
        all_events = new_result.get("events", [])
        if all_events:
            print(f"Total events detected: {len(all_events)}")
            for i, event in enumerate(all_events, 1):
                timing = event.exact_in_days if hasattr(event, 'exact_in_days') else "unknown"
                event_type = event.event_type.value if hasattr(event, 'event_type') else "unknown"
                reason = event.reason if hasattr(event, 'reason') else "no reason"
                print(f"  {i}. {event_type} (t={timing}): {reason}")
        else:
            print("Total events detected: 0")
        
        # Show primary perfection selection
        primary = new_result.get("primary")
        if primary:
            print(f"\nPRIMARY PERFECTION: {primary.get('type', 'unknown')}")
            print(f"  Confidence: {primary.get('confidence', 'unknown')}%")
            print(f"  Reason: {primary.get('reason', 'no reason')}")
            print(f"  Favorable: {primary.get('favorable', 'unknown')}")
            if primary.get('exact_in_days'):
                print(f"  Timing: {primary.get('exact_in_days')} days")
        else:
            print("\nPRIMARY PERFECTION: None found")
            
        # Show secondary perfection if any
        secondary = new_result.get("secondary") 
        if secondary:
            print(f"\nSECONDARY PERFECTION: {secondary.get('type', 'unknown')}")
            print(f"  Reason: {secondary.get('reason', 'no reason')}")
        
        print(f"=== END PERFECTION PROCESS ===\n")
    ...
    def _is_aspect_favorable_enhanced(self, aspect: Aspect, reception_info: Dict[str, Any], chart: HoraryChart, querent: Planet, quesited: Planet) -> Tuple[bool, List[str], bool]:
        ...
        # mixed_reception should NOT bypass penalties completely
    ...
    def _check_malefic_prohibition(self, chart: HoraryChart, planet: Planet, context_house: int = None, earliest_perfection_days: float = None) -> Dict[str, Any]:
        """Check classical prohibition/corruption relative to earliest perfection timing.

        In this implementation, a contact is only considered when chronology is known:
        - `true_prohibition`: malefic contact preempts the earliest perfection.
        - `post_perfection`: malefic contact follows perfection (corruption/complication).
        """
        ...
        # Prohibition is a pre-emption concept; without chronology there is no prohibition verdict.
        if earliest_perfection_days is None:
            return {
        ...
        for aspect in chart.aspects:
            ...
            if target_planet not in malefics:
                continue

            # Only hard contacts are considered for prohibition/corruption
            if aspect.aspect not in {Aspect.CONJUNCTION, Aspect.SQUARE, Aspect.OPPOSITION}:
                continue

            # Timing is required for chronology-based classification
            aspect_timing = getattr(aspect, 'time_to_perfection', None)
            if aspect_timing is None or aspect_timing <= 0:
                continue
            ...
            if target_pos.dignity_score >= 3:
                severity = "severe"
                reasoning += " (well-dignified malefic = strong opposition)"

            if aspect_timing + eps < earliest_perfection_days:
                kind = "true_prohibition"
                reasoning += " (preempts perfection)"
                post_perfection = False
            elif aspect_timing > earliest_perfection_days + eps:
                kind = "post_perfection"
                reasoning += " (post-perfection corruption - enhanced penalty)"
                post_perfection = True
            else:
                # Near-simultaneous events are not treated as prohibition/corruption.
                continue
            ...
            weight = self._calculate_enhanced_malefic_penalty(
                ...
                aspect,
                aspect_timing,
                earliest_perfection_days,
                post_perfection=post_perfection,
            )
        ...
        if eighth_house_ruler and quesited_planet != eighth_house_ruler:
            ...
            for aspect in chart.aspects:
                if ((aspect.planet1 == quesited_planet and aspect.planet2 == eighth_house_ruler) or
                    ...
                    if aspect.aspect in [Aspect.SQUARE, Aspect.OPPOSITION] and aspect.applying:
    ...
    def _calculate_moon_testimony_score(self, chart: HoraryChart, question_analysis: Dict,
        ...
        # 3. Moon swift (speed > 12 deg/day): +1
        if moon_pos.speed > 12.0:
            moon_score += 1
            factors.append(("Moon swift (speed {:.1f} deg/day)".format(moon_pos.speed), 1))
    ...
    def _get_planet_moiety(self, planet: Planet) -> float:
        """Get traditional moiety for planet from centralized config (half of full orb)."""
        try:
            from horary_config import cfg as _cfg
            full = getattr(_cfg().orbs.moieties, planet.value, None)
            if full is None:
...
class HoraryEngine:
    ...
    def judge(self, question: str, settings: Dict[str, Any]) -> Dict[str, Any]:
        ...
        house_system_code = settings.get("house_system_code")
        include_internal_chart = settings.get("include_internal_chart", False)
        ...
        try:
            result = self.engine.judge_question(
                ...
                manual_houses=manual_houses,
                include_internal_chart=include_internal_chart,
                latitude=latitude,
                longitude=longitude,
                location_name=location_name,
                ignore_radicality=ignore_radicality,
                ...
                class AuditChart:
                    def __init__(self, chart_data):
                        ...
                        if isinstance(raw_planets, dict):
                            planet_items = list(raw_planets.items())
                        elif isinstance(raw_planets, list):
                            for idx, item in enumerate(raw_planets):
                                if not isinstance(item, dict):
                                    continue
                                name = item.get('planet') or item.get('name') or f'planet_{idx}'
                                planet_items.append((str(name), item))
                        for planet_name, planet_data in planet_items:
...
def sort_reasoning_for_display(entries):
    """
    ...
    def is_quality_intro(e):
        return ("question classified as" in t(e)) or (e.get("factor") == "quality")

    def is_chart_radical(e):
        return ("chart is radical" in t(e)) or ("radical" in t(e) and "chart" in t(e))

    def is_identified(e):
        txt = t(e)
        return ("querent:" in txt and "quesited:" in txt) or ("identified" in txt)
    ...
    def is_moon_testimony(e):
        txt = t(e)
        return (
            "moon testimony" in txt
            or ("moon" in txt and any(k in txt for k in [
                "translation", "collection", "prohibition",
                "void of course", "voc", "swift", "slow",
                "increasing light", "decreasing light"
            ]))
        )

    def is_end_of_matter_or_4th(e):
        txt = t(e)
        return (
            e.get("factor") == "end_matter"
            or "end of matter" in txt
            or "end-of-matter" in txt
            or re.search(r"\bin (the )?4(th)?( house)?\b", txt)
        )

    def is_debilitated_lx_ruler(e):
        txt = t(e)
        # e.g. "Debilitated L11 ruler", "L7 ruler in fall", etc.
        return (
            "ruler" in txt
            and re.search(r"\bl\d{1,2}\b", txt)
            and any(w in txt for w in ["debilitated", "detriment", "fall", "peregrine"])
        )

    # Subject grouping (planet or Lx) so items stack "one under the other"
    def subject_key(e):
        txt = t(e)
        m = re.search(r"\b(l\d{1,2})\b", txt)
        if m:
    ...
    # Prioritized buckets (lower comes first)
    def bucket(e):
        if is_saturn_7th_consideration(e):  # "at the top"
            return -1
        if is_quality_intro(e):
            return 0
        if is_chart_radical(e):
            return 1
        if is_identified(e):               # "3rd" when the two above exist
            return 2
        if is_moon_testimony(e):           # "in the middle"
            return 3
        if is_debilitated_lx_ruler(e):     # near the end
            return 7
        if is_end_of_matter_or_4th(e):     # very end
            return 8
        # Default: per-planet conditions and generic items
        txt = t(e)
        cond_tokens = (
            "combust", "under beams", "cazimi", "retrograde", "direct",
            "rulership", "exaltation", "triplicity", "term", "face",
            "peregrine", "detriment", "fall",
            "angular", "succedent", "cadent", "hayz", "fast", "slow", "speed"
        )
        return 4 if any(k in txt for k in cond_tokens) else 5

    # Inside a planet/Lx group, keep a tidy, intuitive order
    order_index = {name: i for i, name in enumerate([
        "CAZIMI","COMBUST","UNDER BEAMS","RETROGRADE","DIRECT",
        "RULERSHIP","EXALTATION","TRIPLICITY","TERM","FACE",
        "PEREGRINE","DETRIMENT","FALL",
        "ANGULAR","SUCCEDENT","CADENT","HAYZ","FAST","SLOW","SPEED"
    ])}
    def condition_rank(e):
        txt = (e.get("rule") or e.get("key") or "").upper()
        for token, idx in order_index.items():
            if token in txt:
                return idx
        return 999
    ...
    for i, e in enumerate(entries or []):
        decorated.append((
            bucket(e),           # primary: section position
            subject_key(e),      # group by planet/Lx so items stack under each other
            condition_rank(e),   # tidy order within the group
            i,                   # stability fallback
            e
        ))
    decorated.sort()
    return [e[-1] for e in decorated]
