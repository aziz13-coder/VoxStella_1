...
def _safe_debug_print(*args, **kwargs):
    """Best-effort debug print that never breaks runtime calculations."""
    enabled = str(os.getenv("PERFECTION_DEBUG_STDOUT", "")).strip().lower() in {"1", "true", "yes"}
    if not enabled:
        return
    try:
        builtins.print(*args, **kwargs)
    except (OSError, ValueError) as exc:
        logger.debug("Suppressed perfection_core debug print failure: %s", exc)


# This module contains extensive debug prints in timing/event paths.
# Guard against stdout/stderr handle errors on some Windows runtime contexts.
...
class EventDetector:
    ...
    def _assess_direct_aspect_quality(self, aspect: Aspect, reception_data: Dict, 
        ...
        Policy update: For easy aspects (conjunction, sextile, trine) keep Favorable=True
        and treat cadency strictly as a confidence penalty. Hard aspects may still
        be unfavorable unless strong mutual reception applies.
        """
        challenges = []
        quality_tags = []
        ...
        hard_aspects = {Aspect.SQUARE, Aspect.OPPOSITION}
        is_easy = aspect in easy_aspects
        ...
        # Check for cadent significators (remain as challenges for confidence/wording)
        querent_pos = chart.planets[querent]
        quesited_pos = chart.planets[quesited]
        ...
        if mutual == "identity":
            ...
            # Strong reception mitigates all challenges for favorability
            if "hard_aspect" in challenges:
                challenges.remove("hard_aspect")
                quality_tags = ["easier"]
        elif mutual == "mixed_reception":
            # Mixed reception partially mitigates (do not flip unfavorable by itself)
            quality_tags.append("mitigated by reception")

        # Overall favorability
        # - Easy aspects remain favorable regardless of cadency
        # - Hard aspects are unfavorable unless strong mutual reception
        if is_easy:
            favorable = True
        else:
            favorable = ("hard_aspect" not in challenges) or (mutual in ["mutual_rulership", "mutual_exaltation"])
