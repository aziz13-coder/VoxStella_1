...
def search_live_location_candidates(location_string: str, limit: int = 6, timeout: int = 12) -> List[Dict[str, Any]]:
    """Search live geocoder candidates for a query.

    This is intentionally used only for user-entered query augmentation, not
    for broad atlas sweeps.
    """
    if not location_string or not str(location_string).strip():
        return []
    if _looks_like_serialized_object(location_string):
    ...
    for location in results:
        ...
        name = (
            ...
            or str(getattr(location, "address", "")).split(",")[0]
        )
        country_code = str(address.get("country_code") or "").upper()
        country_name = str(address.get("country") or country_code or "Unknown")
        admin1 = str(address.get("state") or address.get("region") or address.get("county") or "")
        display_name = str(getattr(location, "address", "") or name or location_string).strip()
        ...
        out.append(
            {
                ...
                "raw_type": str(raw.get("type") or raw.get("class") or ""),
            }
        )
    return out
...
class TimezoneManager:
    ...
    def _get_fallback_timezone(self, lat: float, lon: float) -> Optional[str]:
        ...
        # Final fallback: if TimezoneFinder is available and was initialized,
        # try to resolve timezone via the existing instance (no re-instantiation).
        try:
