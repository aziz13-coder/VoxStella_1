...
cross-build contract: an ordinary ``calc_ut`` may overlap a mutation on a
process-global build, while a thread-local build requires the setter and its
dependent calculation to run on the same thread.  Canonical backend modules
import the synchronized ``swisseph`` facade from here so every callable crosses
one re-entrant boundary.  Multi-call stateful operations additionally use the
same-thread transaction context managers below.
"""
