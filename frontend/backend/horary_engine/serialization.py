...
def _phase_label(applying: bool) -> str:
    return "applying" if bool(applying) else "separating"


def _planet_enum_key(name: Any) -> str:
...
def serialize_lunar_aspect(lunar_aspect: Optional[LunarAspect]) -> Optional[Dict]:
    ...
    return {
        ...
        "phase": _phase_label(lunar_aspect.applying),
    }
...
def serialize_chart_for_frontend(
        ...
        aspects_data.append(
            {
                ...
                "phase": _phase_label(aspect.applying),
    ...
    # Propagate house system code when available
    try:
        if hasattr(chart, "house_system_code") and chart.house_system_code:
            result["house_system_code"] = str(chart.house_system_code)
    except Exception:
        pass
...
def deserialize_chart_for_evaluation(data: Dict[str, Any]) -> HoraryChart:
    ...
    for name, p in data.get("planets", {}).items():
        planet_enum = Planet[_planet_enum_key(name)]
    ...
    for a in data.get("aspects", []):
        phase_raw = str(a.get("phase") or a.get("motion") or "").lower()
        applying = a.get("applying")
        if applying is None:
            applying = ("apply" in phase_raw) or (phase_raw == "applying")
        aspects.append(
            AspectInfo(
                planet1=Planet[_planet_enum_key(a["planet1"])],
                planet2=Planet[_planet_enum_key(a["planet2"])],
    ...
    house_rulers = {
        int(h): Planet[_planet_enum_key(r)] for h, r in data.get("house_rulers", {}).items()
    }

    moon_last_aspect = None
    if ml := data.get("moon_last_aspect"):
        ml_phase_raw = str(ml.get("phase") or ml.get("motion") or "").lower()
        ml_applying = ml.get("applying")
        if ml_applying is None:
            ml_applying = ("apply" in ml_phase_raw) or (ml_phase_raw == "applying")
        moon_last_aspect = LunarAspect(
            planet=Planet[_planet_enum_key(ml["planet"])],
            ...
            applying=bool(ml_applying),
        )

    moon_next_aspect = None
    if mn := data.get("moon_next_aspect"):
        mn_phase_raw = str(mn.get("phase") or mn.get("motion") or "").lower()
        mn_applying = mn.get("applying")
        if mn_applying is None:
            mn_applying = ("apply" in mn_phase_raw) or (mn_phase_raw == "applying")
        moon_next_aspect = LunarAspect(
            planet=Planet[_planet_enum_key(mn["planet"])],
