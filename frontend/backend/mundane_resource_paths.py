...
def resolve_mundane_resource_path(filename: str) -> Path:
    normalized_name = str(filename or "").strip()
    if not normalized_name:
        raise ValueError("Mundane resource filename is required")

    candidates: list[Path] = []
    for root in _candidate_backend_roots():
        candidates.append(root / "knowledge" / "mundane" / normalized_name)
        candidates.append(root / "backend" / "knowledge" / "mundane" / normalized_name)
