...
export function transformDashboard(data) {
  ...
  return {
    ...
    compass: (data.compass && typeof data.compass === 'object') ? data.compass : null,
    ascendant: Number.isFinite(Number(data.ascendant)) ? Number(data.ascendant) : null,
    midheaven: Number.isFinite(Number(data.midheaven)) ? Number(data.midheaven) : null,
    planets,
    moon: data.moon || null,
