...
REPLAY_FIXTURE_PATH = Path(__file__).resolve().parents[1] / "tests" / "fixtures" / "forensic_case_replay_slice_5.json"


class ForensicReplaySliceFiveTests(unittest.TestCase):
    ...
    def test_metadata_basis_is_tracked(self):
        cases = {case["id"]: case for case in load_forensic_replay_cases(REPLAY_FIXTURE_PATH)}
        self.assertEqual(cases["twa_flight_800"]["metadata_basis"], "source_explicit_prose_plus_internal_chart_signature")
        self.assertEqual(cases["twa_flight_800"]["metadata_confidence"], "medium")
        self.assertEqual(cases["haiti_earthquake"]["metadata_basis"], "source_explicit_prose_plus_case_context")
        self.assertEqual(cases["haiti_earthquake"]["metadata_confidence"], "medium")
