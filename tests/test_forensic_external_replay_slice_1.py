...
REPLAY_FIXTURE_PATH = Path(__file__).resolve().parents[1] / "tests" / "fixtures" / "forensic_external_replay_slice_1.json"


class ForensicExternalReplaySliceOneTests(unittest.TestCase):
    def test_replay_slice_has_expected_case(self):
        cases = load_forensic_replay_cases(REPLAY_FIXTURE_PATH)
        self.assertEqual([case["id"] for case in cases], ["idaho_four_murders"])

    def test_metadata_basis_is_tracked(self):
        case = load_forensic_replay_cases(REPLAY_FIXTURE_PATH)[0]
        self.assertEqual(case["metadata_basis"], "official_affidavit_event_window_plus_specific_4_17_audio_anchor")
