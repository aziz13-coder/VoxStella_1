...
REPLAY_FIXTURE_PATH = Path(__file__).resolve().parents[1] / "tests" / "fixtures" / "forensic_external_replay_slice_3.json"


class ForensicExternalReplaySliceThreeTests(unittest.TestCase):
    def test_replay_slice_has_expected_case(self):
        cases = load_forensic_replay_cases(REPLAY_FIXTURE_PATH)
        self.assertEqual([case["id"] for case in cases], ["shinzo_abe_assassination"])

    def test_metadata_basis_is_tracked(self):
        case = load_forensic_replay_cases(REPLAY_FIXTURE_PATH)[0]
        self.assertEqual(case["metadata_basis"], "same_day_press_report_with_around_11_30_jst_anchor")
