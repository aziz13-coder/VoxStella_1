...
FIXTURE_PATH = (
    ...
    / "fixtures"
    / "horary_external_cunning_man_source_pass_slice3.json"
)
...
def test_slice3_contains_five_fresh_external_questions():
    cases = _load_cases()
    assert len(cases) == 5


def test_slice3_tracks_all_router_cases_as_aligned_after_family_pass():
    cases = _load_cases()
    assert any(case["source_alignment"] for case in cases)
    assert all(case["source_alignment"] for case in cases)


def test_slice3_engine_observations_match_current_analyzer_behavior():
...
def test_slice3_has_no_remaining_router_mismatches():
    cases = _load_cases()
