...
FIXTURE_PATH = (
    ...
    / "fixtures"
    / "horary_external_cunning_man_source_pass_slice4.json"
)
...
def test_slice4_contains_three_weather_and_timing_cases():
    cases = _load_cases()
    assert len(cases) == 3


def test_slice4_tracks_all_router_cases_as_aligned_after_weather_and_contact_pass():
    cases = _load_cases()
    assert any(case["source_alignment"] for case in cases)
    assert all(case["source_alignment"] for case in cases)


def test_slice4_engine_observations_match_current_analyzer_behavior():
...
def test_slice4_has_no_remaining_router_mismatches():
    cases = {case["id"]: case for case in _load_cases()}
    mismatch_ids = {
        case_id for case_id, case in cases.items() if not case["source_alignment"]
    }

    assert mismatch_ids == set()
