...
FIXTURE_PATH = (
    ...
    / "fixtures"
    / "horary_external_source_pass_slice5.json"
)
...
def test_slice5_contains_three_mixed_external_doctrine_probes():
    cases = _load_cases()
    assert len(cases) == 3


def test_slice5_tracks_full_generalization_after_secular_event_pass():
    cases = _load_cases()
    aligned = {case["id"] for case in cases if case["source_alignment"]}
...
def test_slice5_engine_observations_match_current_analyzer_behavior():
