...
FIXTURE_PATH = REPO_ROOT / "tests" / "fixtures" / "synastry_historical_replay_slice_3.json"


def test_synastry_historical_replay_slice_3_has_expected_cases():
    fixture = load_synastry_historical_replay_cases(path=FIXTURE_PATH)
    ids = {case["id"] for case in fixture["cases"]}
    assert ids == {
        "frank_sinatra_ava_gardner",
        "sartre_beauvoir",
    }


def test_synastry_historical_replay_slice_3_tracks_timed_sources_and_snapshots():
    fixture = load_synastry_historical_replay_cases(path=FIXTURE_PATH)
...
def test_synastry_historical_replay_slice_3_current_baseline():
    fixture = load_synastry_historical_replay_cases(path=FIXTURE_PATH)
    expected = {
        "frank_sinatra_ava_gardner": {
            "status": "aligned",
            "matched": {"attraction", "friction", "burden"},
            "missed_primary": set(),
            "missed_secondary": set(),
        },
        "sartre_beauvoir": {
            "status": "aligned",
            "matched": {"attachment", "growth", "burden"},
            "missed_primary": set(),
            "missed_secondary": set(),
        },
    }
