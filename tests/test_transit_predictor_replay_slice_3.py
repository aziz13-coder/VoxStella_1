...
REPLAY_FIXTURE_PATH = (
    ...
    / "tests"
    / "fixtures"
    / "transit_predictor_replay_slice_3.json"
)
...
class TransitPredictorReplaySliceThreeTests(TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_fixture_tracks_sources_and_time_basis(self):
        for case in self.cases:
            with self.subTest(case=case["id"]):
                ...
                if case["id"] != "sergio_mattarella_president":
    ...
    def test_predictor_keeps_exact_event_row_localized_and_source_signal_present(self):
        for case in self.cases:
            with self.subTest(case=case["id"]):
                data, natal_meta = self._run_case(case)
                ...
                self.assertIsNotNone(event_row, msg=f"{case['id']} series timestamps={ [row.get('timestamp') for row in series[:8]] }")
                ...
                self.assertLessEqual(
                    ...
                    msg=(
                        ...
                        f"event_step_score={event_row.get('step_score')} "
                ...
                first_peak = (data.get("peaks") or [None])[0]
                self.assertIsInstance(first_peak, dict)
                self.assertTrue(
                    first_peak.get("event_type") or first_peak.get("description"),
                    msg=f"{case['id']} peaks={data.get('peaks')}",
                )
