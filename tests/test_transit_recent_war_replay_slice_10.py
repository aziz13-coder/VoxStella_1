...
REPLAY_FIXTURE_PATH = (
    ...
    / "tests"
    / "fixtures"
    / "transit_recent_war_replay_slice_10.json"
)
...
def _measure_recent_war_response(payload):
...
class TransitRecentWarReplaySliceTenTests(TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def _run_case(self, case, transit_datetime):
        ...
        return _measure_recent_war_response(payload), natal_meta

    def test_replay_slice_has_expected_case(self):
        self.assertEqual(
            [case["id"] for case in self.cases],
            ["israel_roaring_lion_broad_strike"],
        )

    def test_fixture_tracks_recent_sources_and_controls(self):
        case = self.cases[0]
    ...
    def test_event_date_keeps_stronger_recent_war_response_cluster_than_controls(self):
        case = self.cases[0]
        ...
        self.assertFalse(
            any(row["event_type"] == "arrest_imprisonment" for row in event["war_rows"]),
            msg=f"unexpected arrest typing in event_war_rows={event['war_rows']}",
        )
        self.assertFalse(
        ...
        for control_datetime in case["control_datetimes"]:
            with self.subTest(control=control_datetime):
                control, _ = self._run_case(case, control_datetime)
                self.assertGreaterEqual(
